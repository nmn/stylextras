...
import {
  ...
  PLAYGROUND_TYPE_LIBS,
  PLAYGROUND_TYPE_PATHS,
  UI_PACKAGE_IMPORTS,
  UI_SOURCE_FILES,
} from './sources'
...
function normalizeSourceFilePath(filename: string) {
  return filename.startsWith('/') ? filename : `/${filename}`
}

function resolveSourceFile(unresolvedPath: string, sourceFilePaths: ReadonlySet<string>) {
  const candidates = [
    unresolvedPath,
    `${unresolvedPath}.ts`,
    `${unresolvedPath}.tsx`,
  ...
  return candidates.find((candidate) => sourceFilePaths.has(candidate))
}

function finalizeTransformedSource(code: string, filename: string) {
      ...
      () => ({
        visitor: {
          ImportDeclaration(importPath: any) {
            if (importPath.node.source.value === '@stylexjs/atoms') {
              importPath.remove()
            }
          },
        },
...
function transformSourceFiles(
  sourceFiles: Record<string, string>,
  additionalSourceFiles: Record<string, string> = {},
) {
  const stylexRules = []
  const transformedFiles: Record<string, string> = {}
  const sourceFilePaths = new Set(
    [...Object.keys(sourceFiles), ...Object.keys(additionalSourceFiles)].map(
      normalizeSourceFilePath,
    ),
  )
  for (const [filename, code] of Object.entries(sourceFiles)) {
    const sourceFilePath = normalizeSourceFilePath(filename)
    ...
    const result = transform(code, {
      ...
      plugins: [
        ...
        [
          ...
          {
            dev: false,
            // Keep modern media features intact in the playground.
            ...
            unstable_moduleResolution: {
              ...
              filePathResolver(importPath: string, sourceFilePath: string) {
                ...
                const packageSourcePath = UI_PACKAGE_IMPORTS[importPath]
                if (packageSourcePath != null) {
                  return resolveSourceFile(packageSourcePath, sourceFilePaths)
                }
                return undefined
              },
              getCanonicalFilePath(filePath: string) {
                return getCanonicalSourceFilePath(filePath)
              },
            },
          },
        ],
    ...
    transformedFiles[filename] = finalizeTransformedSource(result.code, sourceFilePath)
...
function toBundlerFiles(transformedFiles: Record<string, string>) {
  return Object.fromEntries(
    Object.entries(transformedFiles).map(([filename, code]) => {
      const outputPath = normalizeSourceFilePath(filename).replace(/\.(?:ts|tsx|js|jsx)$/, '.js')
      return [outputPath, { code }]
    }),
  )
}

function getCompiledUiPackage() {
  return COMPILED_UI_PACKAGE
}
...
export default function PlaygroundNew() {
  const [_inputFilesOld, _setInputFilesOld] = useQueryParam('inputFiles', JsonParam)
  ...
  const inputFiles: Record<string, string> = decodeObjKeys(_inputFiles)
  ...
  const [transformedFiles, setTransformedFiles] = useState<Record<string, string>>({})
  ...
  function setInputFiles(updatedInputFiles: Record<string, string>, replace: boolean = true) {
    _setInputFiles(encodeObjKeys(updatedInputFiles), replace ? 'replaceIn' : 'push')
  }
  useEffect(() => {
  ...
  function getUniqueFilename(baseName: string) {
  ...
  function createComponentTemplate(filename: string) {
...
`
  ...
  function createStylexVarsTemplate() {
    return `import * as stylex from '@stylexjs/stylex';
...
`
  ...
  function getDefaultFilename(fileKind: string) {
    return fileKind === 'component'
      ? getUniqueFilename('Component.tsx')
      : getUniqueFilename('tokens.stylex.ts')
  }
  function ensureMonacoModel(monacoInstance: any, filename: string, content: string) {
  ...
  function updateSandpack(updatedInputFiles: Record<string, string>) {
    try {
      const uiPackage = getCompiledUiPackage()
      const { transformedFiles, generatedCSS } = transformSourceFiles(
        updatedInputFiles,
        UI_SOURCE_FILES,
      )
  ...
  const createFile = (fileKind: 'component' | 'stylex', name: string) => {
    ...
    const template =
      fileKind === 'component' ? createComponentTemplate(trimmedName) : createStylexVarsTemplate()
    const updatedInputFiles = {
      ...inputFiles,
  ...
  const renameFile = (oldName: string, newName: string) => {
    ...
    const updatedInputFiles = Object.fromEntries(
      Object.entries(inputFiles as Record<string, string>).map(([key, value]) =>
        key === oldName ? [trimmedName, value] : [key, value],
      ),
    )
    const success = updateSandpack(updatedInputFiles)
    setInputFiles(updatedInputFiles, success)
    ensureMonacoModel(monaco, trimmedName, updatedInputFiles[trimmedName] ?? '')
    ...
    <div {...stylex.props(styles.container)}>
      <div {...stylex.props(styles.frame)}>
        <div {...stylex.props(styles.row)}>
                ...
                <Editor
                  beforeMount={(monaco: any) => {
                    ...
                    monaco.languages.typescript.typescriptDefaults.setCompilerOptions({
                      ...
                      paths: PLAYGROUND_TYPE_PATHS,
                    })
                    for (const [file, content] of Object.entries(PLAYGROUND_TYPE_LIBS)) {
                  ...
                  theme={colorMode === 'dark' ? DARK_EDITOR_THEME : LIGHT_EDITOR_THEME}
                />
              </div>
              {error != null && <div {...stylex.props(styles.error)}>{error.message}</div>}
            </Panel>
            <div {...stylex.props(styles.panel, outputCollapsed && styles.panelClosed)}>
                ...
                {!outputCollapsed && (
                  <div {...stylex.props(styles.outputEditor)}>
                    <Editor
                      ...
                      defaultLanguage={activeOutputTab === 'css' ? 'css' : 'javascript'}
                      options={{
                        minimap: { enabled: false },
          ...
          <div {...stylex.props(styles.column)}>
            <CollapsiblePanel alwaysOpen={true} header="Preview" style={styles.previewPanel}>
              <iframe
                ref={iframeRef}
                title="StyleX Playground Preview"
                {...stylex.props(styles.iframe, !sandpackInitialized && styles.iframeHidden)}
              />
            </CollapsiblePanel>
          </div>
        </div>
...
function CollapsiblePanel({
  ...
  return (
    ...
    >
      {header && (
        <button
          {...stylex.props(styles.panelHeader, alwaysOpen && styles.panelHeaderButtonAlwaysOpen)}
          onClick={() => setOpen((open) => !open)}
        >
          {header}
          <ChevronDown {...stylex.props(styles.panelChevron, open && styles.panelChevronOpen)} />
        </button>
      )}
      <div
        {...stylex.props(
