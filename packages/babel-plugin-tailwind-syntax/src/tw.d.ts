...
type SizeFractions = SizeFractionsUpto6 & {
  '1/12': '8.333333%'
  '2/12': '16.666667%'
  '3/12': '25%'
  '4/12': '33.333333%'
...
type AspectRatio<T extends string> = T extends `aspect-[${infer CustomValue}]`
...
type Columns<T extends string> = T extends `columns-[${infer Value}]`
...
type BoxSizing<T extends string> = T extends `box-${infer Value extends 'border' | 'content'}`
  ? { boxSizing: `${Value}-box` }
  : {}
...
type Display<T extends string> = T extends infer Value extends DisplayValue
  ? { display: Value }
  : T extends 'hidden'
    ? { display: 'none' }
    : {}

type Float<T extends string> = T extends `float-${infer Value extends 'start' | 'end'}`
  ? { float: `inline-${Value}` }
  : T extends `float-${infer Value extends 'right' | 'left' | 'none'}`
    ? { float: Value }
    : {}

type Clear<T extends string> = T extends `clear-${infer Value extends 'start' | 'end'}`
  ? { clear: `inline-${Value}` }
  : T extends `clear-${infer Value extends 'right' | 'left' | 'none'}`
    ? { clear: Value }
    : {}

type Isolation<T extends string> = T extends `isolation-${infer Value extends 'auto' | 'isolate'}`
  ? { isolation: Value }
  : {}
...
type ObjectPosition<T extends string> = T extends `object-[${infer CustomValue}]`
...
type Overflow<T extends string> = T extends `overflow-${infer Value extends OverflowValue}`
  ? { overflow: Value }
...
type Position<T extends string> = T extends infer Value extends
  | 'static'
  | 'fixed'
  | 'absolute'
  | 'relative'
...
type InsetValue = SpacingNumberToLength & {
  '1/2': '50%'
  '1/3': '33.333333%'
  '2/3': '66.666667%'
  '1/4': '25%'
...
type Inset<T extends string> = T extends `inset-x-[${infer CustomValue}]`
...
type Visibility<T extends string> = T extends 'visible'
  ? { visibility: 'visible' }
  : T extends 'invisible'
    ? { visibility: 'hidden' }
    : T extends 'collapse'
...
type ZIndex<T extends string> = T extends `z-[${infer CustomValue}]`
  ? { zIndex: CustomValue }
  : T extends `z-${infer Value extends 0 | 10 | 20 | 30 | 40 | 50 | 'auto'}`
    ? { zIndex: Value }
    : {}

type FlexBasis<T extends string> = T extends `basis-[${infer CustomValue}]`
  ? { flexBasis: UnderscoreToSpace<CustomValue> }
  : T extends `basis-${infer Value extends InsetValueKey}`
    ? { flexBasis: InsetValue[Value] }
    : {}

type FlexDirection<T extends string> = T extends 'flex-row'
  ? { flexDirection: 'row' }
...
type FlexWrap<T extends string> = T extends 'flex-wrap'
  ? { flexWrap: 'wrap' }
...
type Flex<T extends string> = T extends `flex-[${infer CustomValue}]`
...
type FlexGrow<T extends string> = T extends `grow-[${infer CustomValue}]`
...
type FlexShrink<T extends string> = T extends `shrink-[${infer CustomValue}]`
...
type FlexOrder<T extends string> = T extends `order-[${infer CustomValue}]`
...
type GridTemplateColumns<T extends string> = T extends `grid-cols-[${infer CustomValue}]`
...
type GridColumnStart<T extends string> = T extends `col-start-[${infer CustomValue}]`
...
type GridColumnEnd<T extends string> = T extends `col-end-[${infer CustomValue}]`
...
type GridColumnSpan<T extends string> = T extends `col-span-${infer Value extends number}`
  ? { gridColumn: `span ${Value} / span ${Value}` }
  : T extends `col-span-full`
    ? { gridColumn: '1 / -1' }
    : {}

type GridColumn<T extends string> = T extends `col-[${infer CustomValue}]`
  ? { gridColumn: UnderscoreToSpace<CustomValue> }
  : T extends `col-auto`
    ? { gridColumn: 'auto' }
    : {}

type GridTemplateRows<T extends string> = T extends `grid-rows-[${infer CustomValue}]`
...
type GridRowStart<T extends string> = T extends `row-start-[${infer CustomValue}]`
...
type GridRowEnd<T extends string> = T extends `row-end-[${infer CustomValue}]`
...
type GridRowSpan<T extends string> = T extends `row-span-${infer Value extends number}`
  ? { gridRow: `span ${Value} / span ${Value}` }
  : T extends `row-span-full`
    ? { gridRow: '1 / -1' }
    : {}

type GridRow<T extends string> = T extends `row-[${infer CustomValue}]`
  ? { gridRow: UnderscoreToSpace<CustomValue> }
  : T extends `row-auto`
    ? { gridRow: 'auto' }
    : {}

type GridAutoFlow<T extends string> = T extends `grid-flow-row`
  ? { gridAutoFlow: 'row' }
...
type GridAutoColumns<T extends string> = T extends `auto-cols-[${infer CustomValue}]`
...
type GridAutoRows<T extends string> = T extends `auto-rows-[${infer CustomValue}]`
...
type Gap<T extends string> = T extends `gap-[${infer CustomValue}]`
...
type RowGap<T extends string> = T extends `gap-y-[${infer CustomValue}]`
...
type ColumnGap<T extends string> = T extends `gap-x-[${infer CustomValue}]`
...
type Padding<T extends string> = T extends `p-[${infer CustomValue}]`
...
type PaddingTop<T extends string> = T extends `pt-[${infer CustomValue}]`
...
type PaddingRight<T extends string> = T extends `pr-[${infer CustomValue}]`
...
type PaddingBottom<T extends string> = T extends `pb-[${infer CustomValue}]`
...
type PaddingLeft<T extends string> = T extends `pl-[${infer CustomValue}]`
...
type PaddingInlineStart<T extends string> = T extends `ps-[${infer CustomValue}]`
...
type PaddingInlineEnd<T extends string> = T extends `pe-[${infer CustomValue}]`
...
type PaddingInline<T extends string> = T extends `px-[${infer CustomValue}]`
...
type PaddingBlock<T extends string> = T extends `py-[${infer CustomValue}]`
...
type Margin<T extends string> = T extends `m-[${infer CustomValue}]`
...
type MarginTop<T extends string> = T extends `mt-[${infer CustomValue}]`
...
type MarginRight<T extends string> = T extends `mr-[${infer CustomValue}]`
...
type MarginBottom<T extends string> = T extends `mb-[${infer CustomValue}]`
...
type MarginLeft<T extends string> = T extends `ml-[${infer CustomValue}]`
...
type MarginInlineStart<T extends string> = T extends `ms-[${infer CustomValue}]`
...
type MarginInlineEnd<T extends string> = T extends `me-[${infer CustomValue}]`
...
type MarginInline<T extends string> = T extends `mx-[${infer CustomValue}]`
...
type MarginBlock<T extends string> = T extends `my-[${infer CustomValue}]`
...
type Width<T extends string> = T extends `w-[${infer CustomValue}]`
...
type MinWidth<T extends string> = T extends `min-w-[${infer CustomValue}]`
...
type MaxWidth<T extends string> = T extends `max-w-[${infer CustomValue}]`
...
type Height<T extends string> = T extends `h-[${infer CustomValue}]`
...
type MinHeight<T extends string> = T extends `min-h-[${infer CustomValue}]`
...
type MaxHeight<T extends string> = T extends `max-h-[${infer CustomValue}]`
...
type Size<T extends string> = T extends `size-[${infer CustomValue}]`
  ? {
      width: UnderscoreToSpace<CustomValue>
      height: UnderscoreToSpace<CustomValue>
    }
...
type Font<T extends string> = T extends `font-[${infer Value}]`
...
type FontSize<T extends string> = T extends `text-[${infer CustomValue}]`
...
type FontSmoothing<T extends string> = T extends 'antialiased'
  ? { WebkitFontSmoothing: 'antialiased'; MozOsxFontSmoothing: 'grayscale' }
  : T extends 'subpixel-antialiased'
    ? { WebkitFontSmoothing: 'auto'; MozOsxFontSmoothing: 'auto' }
    : {}

type FontStyle<T extends string> = T extends 'italic'
  ? { fontStyle: 'italic' }
...
type FontWeight<T extends string> = T extends `font-${infer Value extends keyof FontWeightValue}`
  ? { fontWeight: FontWeightValue[Value] }
  : {}

type FontVariantNumeric<T extends string> = T extends 'normal-nums'
...
type LetterSpacing<T extends string> = T extends `tracking-[${infer CustomValue}]`
...
type LineClamp<T extends string> = T extends `clamp-[${infer Value extends number}]`
  ? {
      overflow: 'hidden'
      display: '-webkit-box'
...
type LineHeight<T extends string> = T extends `leading-[${infer CustomValue}]`
...
type ListStyleImage<T extends string> = T extends `list-image-[${infer CustomValue}]`
...
type ListStylePosition<T extends string> =
  T extends `list-${infer Value extends 'inside' | 'outside'}` ? { listStylePosition: Value } : {}

type ListStyleType<T extends string> = T extends `list-[${infer CustomValue}]`
...
type TextDecoration<T extends string> = T extends infer Value extends
  | 'underline'
  | 'line-through'
  | 'overline'
...
type TextDecorationThickness<T extends string> = T extends `decoration-[${infer CustomValue}]`
...
type TextUnderlineOffset<T extends string> = T extends `underline-offset-[${infer CustomValue}]`
...
type TextTransform<T extends string> = T extends 'uppercase' | 'lowercase' | 'capitalize'
  ? { textTransform: T }
  : T extends 'normal-case'
    ? { textTransform: 'none' }
    : {}

type TextOverflow<T extends string> = T extends 'truncate'
  ? { overflow: 'hidden'; textOverflow: 'ellipsis'; whiteSpace: 'nowrap' }
  : T extends `text-${infer Value extends 'ellipsis' | 'clip'}`
    ? { textOverflow: Value }
    : {}

type TextWrap<T extends string> = T extends 'wrap' | 'nowrap' | 'balance' | 'pretty'
  ? { textWrap: T }
  : {}

type TextIndent<T extends string> = T extends `indent-[${infer CustomValue}]`
...
type VerticalAlign<T extends string> = T extends `align-[${infer CustomValue}]`
...
type WordBreakValue = { all: 'break-all'; keep: 'keep-all' }
type WordBreak<T extends string> = T extends `break-normal`
...
type Hyphens<T extends string> =
  T extends `hyphens-${infer Value extends 'none' | 'manual' | 'auto'}` ? { hyphens: Value } : {}
...
type BackgroundColor<T extends string> = T extends `bg-[${infer CustomValue}]`
...
type BackgroundPosition<T extends string> = T extends `bg-[${infer CustomValue}]`
...
type BackgroundSize<T extends string> = T extends `bg-[length:${infer CustomValue}]`
  ? { backgroundSize: UnderscoreToSpace<CustomValue> }
  : T extends `bg-${infer Value extends 'auto' | 'cover' | 'contain'}`
    ? { backgroundSize: Value }
    : {}

type BackgroundImage<T extends string> = T extends `bg-[url(${infer CustomValue})]`
...
type BorderRadius<T extends string> = T extends `rounded-[${infer CustomValue}]`
  ? { borderRadius: UnderscoreToSpace<CustomValue> }
  : T extends `rounded-s-[${infer CustomValue}]`
    ? {
        borderStartStartRadius: UnderscoreToSpace<CustomValue>
        borderEndStartRadius: UnderscoreToSpace<CustomValue>
      }
    : T extends `rounded-e-[${infer CustomValue}]`
      ? {
          borderStartEndRadius: UnderscoreToSpace<CustomValue>
          borderEndEndRadius: UnderscoreToSpace<CustomValue>
        }
                                          ...
                                          : T extends `rounded-br`
                                            ...
                                            : T extends `rounded-${infer Value extends keyof BorderRadiusValue}`
                                              ...
                                              : T extends `rounded-s-${infer Value extends keyof BorderRadiusValue}`
                                                ? {
                                                    borderStartStartRadius: BorderRadiusValue[Value]
                                                    borderEndStartRadius: BorderRadiusValue[Value]
                                                  }
                                                : T extends `rounded-e-${infer Value extends keyof BorderRadiusValue}`
                                                  ? {
                                                      borderStartEndRadius: BorderRadiusValue[Value]
                                                      borderEndEndRadius: BorderRadiusValue[Value]
                                                    }
...
type TwClassToStyle<T extends string> = AspectRatio<T> &
  Columns<T> &
  BreakAfter<T> &
  BreakBefore<T> &
  BreakInside<T> &
...
type MediaQueries = 'sm' | 'md' | 'lg' | 'xl' | '2xl' | 'dark'
...
type RemoveConditions<T extends string[]> = T extends [infer First, ...infer Rest]
...
type SpaceSeparated<T extends string> = T extends `${infer FirstWord} ${infer Rest}`
...
type MergeObjectsWithUnion<T extends { [key: string]: any }[]> = T extends [
  infer First,
  ...infer Rest,
]
  ? First extends Record<string, any>
    ? Rest extends { [key: string]: any }[]
      ? {
          [K in keyof First | keyof MergeObjectsWithUnion<Rest>]: K extends keyof First
...
type TwToObj<T extends string> = Resolve<
  MergeObjectsWithUnion<TupleToObjects<RemoveConditions<SpaceSeparated<T>>>>
>

type Input1 =
  'flex flex-col gap-x-2.5 justify-start min-h-dvh relative bottom-3/4 aspect-[22_/_9] basis-5 basis-4'
type Input2 = 'font-sans tracking-wider hyphens-auto break-normal'
type InputX =
  'flex flex-col gap-x-2.5 md:gap-px justify-start min-h-dvh after:relative absolute lg:sticky bottom-3/4 hover:aspect-[22_/_9] basis-5 dark:basis-4'
