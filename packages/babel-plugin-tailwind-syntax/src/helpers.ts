...
export type JssValue = string | null | { default: JssValue; [key: string]: JssValue }
...
function _deepEquals(a: unknown, b: unknown): boolean {
  ...
  for (const key of aKeys) {
    if (!_deepEquals((a as Record<string, unknown>)[key], (b as Record<string, unknown>)[key])) {
      return false
    }
  }
  return true
...
function deepEquals([a, ...rest]: ReadonlyArray<unknown>): boolean {
  for (const b of rest) {
    if (!_deepEquals(a, b)) {
      return false
    }
...
const toPack: ReadonlyArray<[ReadonlyArray<string>, string]> = [
  ...
  try {
    ...
    const processNode = (node: any, atRules: ReadonlyArray<string> = []) => {
      ...
      if (node.type === 'rule') {
        ...
        const className = pseudos.reduce(
          (acc, pseudo) => acc.replace(pseudo, ''),
          node.selector.replace(/^\./, '').replaceAll(' .', '').replaceAll('\\', ''),
        )
      ...
      if (node.type === 'decl') {
        ...
        if (pseudoElementIndexes.length > 0) {
          const lastPseudoElementIndex = pseudoElementIndexes[pseudoElementIndexes.length - 1]
          ...
          deeplyAddValue(object, node.value, [
            ...untilLastPseudoElement,
            propName,
            ...afterLastPseudoElement,
          ])
        } else {
