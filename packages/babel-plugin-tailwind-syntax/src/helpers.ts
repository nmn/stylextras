...
function packObject(obj: Jss) {
  ...
  for (const [toFind, replacement] of toPack) {
    const allExist = toFind.every(
      (key) => obj[key] !== undefined && !keysToSkip.includes(key)
    );
    if (!allExist) {
      continue;
    }
    ...
    const processNode = (node: any, atRules: ReadonlyArray<string> = []) => {
      ...
      if (node.type === "decl") {
        ...
        if (pseudoElementIndexes.length > 0) {
          const lastPseudoElementIndex =
            pseudoElementIndexes[pseudoElementIndexes.length - 1];
          const untilLastPseudoElement = atRules.slice(
            0,
            lastPseudoElementIndex + 1
          );
          const afterLastPseudoElement = atRules.slice(
            lastPseudoElementIndex + 1
          );
